...
Every audience view declares both `quantitative-progress` and `flow-progress`. The modes change presentation only. Internal panels may add `action-ledger` and `risk-register` utility routes after the three audience views; they present the sealed `actions/action-ledger.md` and `views/risk-matrix.md` previews without adding section bindings, audience contracts, or a second data model. Shareable panels omit both routes.
...
The node drawer groups explicit related references as decisions, to-dos, open questions, and risks. Flow `action`/`risk` source refs supply to-do/risk items; a meeting-board decision or open question appears on a node only when that structured item carries the node ID in `related_plan_item_ids`. The renderer never relates items by owner, workstream, ID prefix, or text similarity. Full-screen mode changes presentation only; Left/Right cycles through the filtered nodes whose canonical execution is `in-progress`, without changing model state or selection identity. Canvas zoom is clamped to `0.35..8`; pointer pan converts client movement into SVG coordinates and applies a fixed 1.35 gain. These controls change only the transient viewport transform.

Internal meeting items expose a source-file action only when `Source` provides a memory-root-relative path, either directly, through structured `artifact_path`, or through the legacy Python mapping string emitted by older meeting packs. Legacy strings are never executed; the renderer extracts only `artifact_path`, then applies the same scheme, absolute-path, drive-path, and traversal rejection used for structured sources. Referenced UTF-8 Markdown is embedded as escaped source data, limited to 32 files and 256 KiB each, then rendered by the pinned offline markdown-it runtime with raw HTML, images, and link navigation disabled. Action ledger and risk matrix have first priority within that fixed file limit. The normalized source fingerprint participates in `panel_id`; the artifact audit validates the source projection and exact renderer bytes. Shareable output carries neither preview fingerprints nor content. Structured `Action ID|action_id` and `Risk ID|risk_id` values may expose exact utility-view jumps; the renderer never infers register membership from an ID prefix or free text. `risk-matrix.md` publishes canonical `risk_id` in its first column so the risk jump remains an exact ID join. The test-only `--fixture` mode materializes referenced files so the journey proves readable preview and exact register navigation rather than only a plausible path.
