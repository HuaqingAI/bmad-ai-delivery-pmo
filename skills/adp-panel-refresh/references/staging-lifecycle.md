...
- the transitive files declared by those receipts, including historical-input-change `migration.original_input_snapshot_path`, in-memory `migration.evidence_path`, and retirement successor receipt bindings.

`input-manifest.json` records every copied relative path, SHA-256, size, source type, the plan's declared source fingerprints, and a content ID. Status-sync terminal receipts and their reachable closure proof are plan-bound sources; manifest types distinguish terminal receipts, migration originals, migration evidence, and retirement successors. These files are not optional history because state audit recomputes the original input hash, executable diff, and successor validity when proving canonical intake closure. Other unreferenced old receipts, transactions, unrelated audits, Management Panel bundles, and prior inspect output remain excluded.
