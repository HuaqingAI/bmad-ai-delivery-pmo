...
For `never-applied` or `invalid-proposal`, omit `--superseded-by` and provide `--justification <governance-rationale>`. Those reasons fail closed if canonical reconciliation is unavailable or any command is already supported by current facts or lineage. `superseded-by` binds the exact successor intake bytes or a validated durable status-sync receipt; changing the successor invalidates retirement during audit. Apply with the unchanged arguments and returned 15-minute token, replacing `--dry-run` with `--token <single-use-token>`.
