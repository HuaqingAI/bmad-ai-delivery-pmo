...
A historical value may be `satisfied_by: superseded-lineage` only when durable execution receipts or ordered daily-log entries prove the old value and a later current value, or when exact action command/provenance bindings uniquely map a normalized source to the current action ID. An exact historical `change_notes` value may additionally use a verified current WDR whose only suffix is fully matched structured checkpoint/risk metadata such as `risk_id`, `baseline_revision`, `related_plan_item_ids`, or a `Candidate CHK-* from <artifact>:<path>` marker. A physical action route may normalize to `program` only when the stable action ID matches, the historical workstream is an exact current `affected_workstreams` member, and the same source/status/action ID appears in an ordered daily log. Missing history or affected-workstream provenance remains partial; source comparison is never dropped and fuzzy text matching or attestation is never accepted.
